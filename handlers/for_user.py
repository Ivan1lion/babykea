...
async def successful_payment(message: Message, state: FSMContext, session: AsyncSession()):
    ...
    await message.answer(text=f"ВАШ ЗАПРОС ОТПРАВЛЕН\n"
                              f"\nПредложения от продавцов (при наличии искомых комплектующих) могут поступать в "
                              f"течении 24 часов (т.к. запрос осуществляется по всей стране "
                              f"и продавцы могут находиться в разных часовых поясах)\n"
                              f"\n❗️ Проверьте что у вас включено разрешение на входящие сообщения, иначе продавцы "
                              f"не смогут с вами связаться\n"
                              f"\nшаг 1 -> ⚙️ Настройки"
                              f"\nшаг 2 -> 🔒 Конфиденциальность"
                              f"\nшаг 3 -> Сообщения (Все)")
    await state.clear()
